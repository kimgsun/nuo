...
| fs-3xl | 1.875rem | 700 | 섹션 타이틀 |
| fs-2xl | 1.5rem | 700 | 서브 타이틀 |
| fs-lg | 1.125rem | 200 | 강조 본문 |
| fs-base | 1rem | 400 | 일반 본문 |
| fs-sm | 0.875rem | 400 | 캡션 |
...
- **SCSS**: abstracts / layout / components / pages 모듈화, BEM 간소화(visual__box 수준), 변수·Mixin으로 중복 최소화, 미디어 쿼리 단축(@include t/m/d), 속성 순서(레이아웃 → 박스모델 → 시각효과 → 기타).
